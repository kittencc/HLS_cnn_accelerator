...
#ifdef  CCS_DESIGN_FUNC_InputDoubleBuffer_16384_4_4_run
extern void mc_testbench_capture_IN( ac_channel<ac_int<8, true > > &inputs_in,  ac_channel<PackedInt<8UL, 4UL > > &inputs_out,  ac_channel<Params > &paramsIn);
extern void mc_testbench_capture_OUT( ac_channel<ac_int<8, true > > &inputs_in,  ac_channel<PackedInt<8UL, 4UL > > &inputs_out,  ac_channel<Params > &paramsIn);
...
template<>
struct ccs_class_spec<16384,4,4> {
...
class ccs_intercept
{
  ...
  void capture_IN( ac_channel<ac_int<8, true > > &inputs_in,ac_channel<PackedInt<8UL, 4UL > > &inputs_out,ac_channel<Params > &paramsIn ) {}
  void capture_OUT( ac_channel<ac_int<8, true > > &inputs_in,ac_channel<PackedInt<8UL, 4UL > > &inputs_out,ac_channel<Params > &paramsIn ) {}
  void wait_for_idle_sync() {}
  ccs_intercept() {}
};

template<>
class ccs_intercept<16384,4,4>
{
  public:
  void capture_THIS( void* _this ) {
    ...
    if ( capture_msg && dut != NULL && dut == cur_inst ) {
      std::cout << "SCVerify intercepting C++ function 'InputDoubleBuffer<16384, 4, 4>::run' for RTL block 'InputDoubleBuffer_16384_4_4'" << std::endl;
      capture_msg = false;
    }
  }
  void capture_IN(  ac_channel<ac_int<8, true > > &inputs_in,  ac_channel<PackedInt<8UL, 4UL > > &inputs_out,  ac_channel<Params > &paramsIn ) {
...
#define ccs_intercept_run_183 \
  run(ac_channel<ac_int<8, true > > &inputs_in,ac_channel<PackedInt<8UL, 4UL > > &inputs_out,ac_channel<Params > &paramsIn) {\
...
#endif //CCS_DESIGN_FUNC_InputDoubleBuffer_16384_4_4_run
