...
void testbench::exec_run( ac_channel<ac_int<8, true > > &inputs_in,  ac_channel<PackedInt<8UL, 4UL > > &inputs_out,  ac_channel<Params > &paramsIn) {
...
void mc_testbench::capture_inputs_out( ac_channel<PackedInt<8UL, 4UL > > &inputs_out)
{
   static mc_golden_info< mgc_sysc_ver_array1D<ac_int<8, true >,4>, ac_int<8, true > > inputs_out_value_tmp(testbench::inputs_out_value_ignore, false, inputs_out_value_iteration_count);
   ...
   inputs_out_value_tmp._data.mc_testbench_process_array_bounds("inputs_out_value",testbench::inputs_out_value_array_comp_first,testbench::inputs_out_value_array_comp_last,3,0);
   ...
   if (_capture_inputs_out_value)
   {
      ...
      while (ccs_scv_i < inputs_out.ac_channel<PackedInt<8UL, 4UL > >::debug_size()) {
         for (ccs_scv_j = last_j_inputs_out_value_out; ccs_scv_j < chan_factor; ccs_scv_j++,ccs_scv_i++) {
            if (ccs_scv_i < inputs_out.ac_channel<PackedInt<8UL, 4UL > >::debug_size()) {
               int inputs_out_linear_idx = 0;
               for (int inputs_out_idx_0 = 0; inputs_out_idx_0 < 4; ++inputs_out_idx_0)
                  inputs_out_value_tmp._data[inputs_out_linear_idx++] = inputs_out.ac_channel<PackedInt<8UL, 4UL > >::chan[ccs_scv_i].PackedInt<8UL, 4UL >::value[inputs_out_idx_0];
            } else {
               break;
            }
         }
...
void mc_testbench::capture_IN( ac_channel<ac_int<8, true > > &inputs_in,  ac_channel<PackedInt<8UL, 4UL > > &inputs_out,  ac_channel<Params > &paramsIn)
{
   that->capture_inputs_in(inputs_in);
   that->remaining_inputs_out_value_golden =  inputs_out.ac_channel<PackedInt<8UL, 4UL > >::debug_size();
...
void mc_testbench::capture_OUT( ac_channel<ac_int<8, true > > &inputs_in,  ac_channel<PackedInt<8UL, 4UL > > &inputs_out,  ac_channel<Params > &paramsIn)
{
...
void mc_testbench::exec_run( ac_channel<ac_int<8, true > > &inputs_in,  ac_channel<PackedInt<8UL, 4UL > > &inputs_out,  ac_channel<Params > &paramsIn)
{
   #ifndef CCS_SCVERIFY_USE_CCS_BLOCK
   that->cpp_testbench_active.write(true);
   capture_IN(inputs_in, inputs_out, paramsIn);
   InputDoubleBuffer<16384, 4, 4>::run(inputs_in, inputs_out, paramsIn);
