...
#ifdef  CCS_DESIGN_FUNC_InputDoubleBuffer_4096_16_16_run
extern void mc_testbench_capture_IN( ac_channel<ac_int<8, true > > &inputs_in,  ac_channel<PackedInt<8UL, 16UL > > &inputs_out,  ac_channel<Params > &paramsIn);
extern void mc_testbench_capture_OUT( ac_channel<ac_int<8, true > > &inputs_in,  ac_channel<PackedInt<8UL, 16UL > > &inputs_out,  ac_channel<Params > &paramsIn);
...
class ccs_intercept
{
  ...
  void capture_IN( ac_channel<ac_int<8, true > > &inputs_in,ac_channel<PackedInt<8UL, 16UL > > &inputs_out,ac_channel<Params > &paramsIn ) {}
  void capture_OUT( ac_channel<ac_int<8, true > > &inputs_in,ac_channel<PackedInt<8UL, 16UL > > &inputs_out,ac_channel<Params > &paramsIn ) {}
...
class ccs_intercept<4096,16,16>
{
  ...
  void capture_THIS( void* _this ) {
    ...
    if ( capture_msg && dut != NULL && dut == cur_inst ) {
      std::cout << "SCVerify intercepting C++ function 'InputDoubleBuffer<4096, 16, 16>::run' for RTL block 'InputDoubleBuffer_4096_16_16'" << std::endl;
      capture_msg = false;
    }
  }
  void capture_IN(  ac_channel<ac_int<8, true > > &inputs_in,  ac_channel<PackedInt<8UL, 16UL > > &inputs_out,  ac_channel<Params > &paramsIn ) {
    if ( dut != NULL && dut == cur_inst )
      mc_testbench_capture_IN(inputs_in,inputs_out,paramsIn);
  }
  void capture_OUT(  ac_channel<ac_int<8, true > > &inputs_in,  ac_channel<PackedInt<8UL, 16UL > > &inputs_out,  ac_channel<Params > &paramsIn ) {
    if ( dut != NULL && dut == cur_inst )
      mc_testbench_capture_OUT(inputs_in,inputs_out,paramsIn);
  }
  void wait_for_idle_sync() {
    if ( dut != NULL && dut == cur_inst )
...
#define ccs_intercept_run_183 \
  run(ac_channel<ac_int<8, true > > &inputs_in,ac_channel<PackedInt<8UL, 16UL > > &inputs_out,ac_channel<Params > &paramsIn) {\
    ...
    ccs_intercept_inst.capture_IN(inputs_in,inputs_out,paramsIn);\
    ccs_real_run(inputs_in,inputs_out,paramsIn);\
    ccs_intercept_inst.capture_OUT(inputs_in,inputs_out,paramsIn);\
  }\
  void ccs_real_run
#else
#define ccs_intercept_run_183 run
#endif //CCS_DESIGN_FUNC_InputDoubleBuffer_4096_16_16_run
