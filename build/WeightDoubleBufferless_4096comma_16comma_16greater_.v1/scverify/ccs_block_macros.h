...
#ifdef  CCS_DESIGN_FUNC_WeightDoubleBuffer_4096_16_16_run
extern void mc_testbench_capture_IN( ac_channel<ac_int<8, true > > &weights_in,  ac_channel<PackedInt<8UL, 16UL > > &weights_out,  ac_channel<Params > &paramsIn);
extern void mc_testbench_capture_OUT( ac_channel<ac_int<8, true > > &weights_in,  ac_channel<PackedInt<8UL, 16UL > > &weights_out,  ac_channel<Params > &paramsIn);
...
template<>
struct ccs_class_spec<4096,16,16> {
...
class ccs_intercept
{
  ...
  void capture_IN( ac_channel<ac_int<8, true > > &weights_in,ac_channel<PackedInt<8UL, 16UL > > &weights_out,ac_channel<Params > &paramsIn ) {}
  void capture_OUT( ac_channel<ac_int<8, true > > &weights_in,ac_channel<PackedInt<8UL, 16UL > > &weights_out,ac_channel<Params > &paramsIn ) {}
  void wait_for_idle_sync() {}
  ccs_intercept() {}
};

template<>
class ccs_intercept<4096,16,16>
{
  public:
  void capture_THIS( void* _this ) {
    ...
    if ( capture_msg && dut != NULL && dut == cur_inst ) {
      std::cout << "SCVerify intercepting C++ function 'WeightDoubleBuffer<4096, 16, 16>::run' for RTL block 'WeightDoubleBuffer_4096_16_16'" << std::endl;
      capture_msg = false;
    }
  }
  void capture_IN(  ac_channel<ac_int<8, true > > &weights_in,  ac_channel<PackedInt<8UL, 16UL > > &weights_out,  ac_channel<Params > &paramsIn ) {
    if ( dut != NULL && dut == cur_inst )
      mc_testbench_capture_IN(weights_in,weights_out,paramsIn);
  }
  void capture_OUT(  ac_channel<ac_int<8, true > > &weights_in,  ac_channel<PackedInt<8UL, 16UL > > &weights_out,  ac_channel<Params > &paramsIn ) {
    if ( dut != NULL && dut == cur_inst )
      mc_testbench_capture_OUT(weights_in,weights_out,paramsIn);
  }
  void wait_for_idle_sync() {
    if ( dut != NULL && dut == cur_inst )
...
#define ccs_intercept_run_162 \
  run(ac_channel<ac_int<8, true > > &weights_in,ac_channel<PackedInt<8UL, 16UL > > &weights_out,ac_channel<Params > &paramsIn) {\
    static ccs_intercept<size,IC0,OC0> ccs_intercept_inst;\
    void* ccs_intercept_this = this;\
    ccs_intercept_this = ccs_class_spec<size,IC0,OC0>::get(ccs_intercept_this);\
    ccs_intercept_inst.capture_THIS(ccs_intercept_this);\
    ccs_intercept_inst.capture_IN(weights_in,weights_out,paramsIn);\
    ccs_real_run(weights_in,weights_out,paramsIn);\
    ccs_intercept_inst.capture_OUT(weights_in,weights_out,paramsIn);\
  }\
  void ccs_real_run
#else
#define ccs_intercept_run_162 run
#endif //CCS_DESIGN_FUNC_WeightDoubleBuffer_4096_16_16_run
