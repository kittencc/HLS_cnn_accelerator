...
bool testbench::weights_in_ignore;
void mc_testbench_weights_in_skip(bool v) { testbench::weights_in_skip = v; }
bool testbench::weights_in_skip;
bool testbench::weights_in_skip_quiet;
bool testbench::weights_in_skip_once;
bool testbench::weights_in_skip_noerr;
int  testbench::weights_in_array_comp_first;
int  testbench::weights_in_array_comp_last;
mc_wait_ctrl testbench::weights_in_wait_ctrl;
bool testbench::weights_out_value_ignore;
void mc_testbench_weights_out_value_skip(bool v) { testbench::weights_out_value_skip = v; }
bool testbench::weights_out_value_skip;
bool testbench::weights_out_value_skip_quiet;
bool testbench::weights_out_value_skip_once;
bool testbench::weights_out_value_skip_noerr;
int  testbench::weights_out_value_array_comp_first;
int  testbench::weights_out_value_array_comp_last;
bool testbench::weights_out_value_use_mask;
ac_int<8, true > testbench::weights_out_value_output_mask;
mc_wait_ctrl testbench::weights_out_value_wait_ctrl;
...
void testbench::exec_run( ac_channel<ac_int<8, true > > &weights_in,  ac_channel<PackedInt<8UL, 16UL > > &weights_out,  ac_channel<Params > &paramsIn) {
   return mc_testbench::exec_run(weights_in, weights_out, paramsIn);
}
#endif
...
// ============================================
// Function: capture_weights_in
// --------------------------------------------

void mc_testbench::capture_weights_in( ac_channel<ac_int<8, true > > &weights_in)
{
   static ac_int<8, true > weights_in_tmp;
   static int last_j_weights_in_in = 0;
   if (weights_in_iteration_count == wait_cnt)
      wait_on_input_required();
   if (_capture_weights_in && !testbench::weights_in_ignore)
   {
      int cur_iter = weights_in_capture_count;
      ++weights_in_iteration_count;
      unsigned int chan_factor = 1;
      unsigned int ccs_scv_i,ccs_scv_j;
      ccs_scv_i = remaining_ccs_weights_in;
      while (ccs_scv_i < weights_in.ac_channel<ac_int<8, true > >::debug_size()) {
         for (ccs_scv_j = last_j_weights_in_in; ccs_scv_j < chan_factor; ccs_scv_j++,ccs_scv_i++) {
            if (ccs_scv_i < weights_in.ac_channel<ac_int<8, true > >::debug_size()) {
               weights_in_tmp = weights_in.ac_channel<ac_int<8, true > >::chan[ccs_scv_i];
            } else {
               break;
            }
         }
         if (ccs_scv_j == chan_factor) {
            ccs_weights_in->put(weights_in_tmp);
            last_j_weights_in_in = 0;
            weights_in_capture_count += chan_factor;
         } else {
            last_j_weights_in_in = ccs_scv_j;
         }
      }
      mc_testbench_util::process_wait_ctrl("weights_in",testbench::weights_in_wait_ctrl,ccs_wait_ctrl_weights_in.operator->(),cur_iter,weights_in_capture_count,1);
   }
   testbench::weights_in_ignore = false;
}
// ============================================
// Function: capture_weights_out
// --------------------------------------------

void mc_testbench::capture_weights_out( ac_channel<PackedInt<8UL, 16UL > > &weights_out)
{
   static mc_golden_info< mgc_sysc_ver_array1D<ac_int<8, true >,16>, ac_int<8, true > > weights_out_value_tmp(testbench::weights_out_value_ignore, false, weights_out_value_iteration_count);
   weights_out_value_tmp._ignore = testbench::weights_out_value_ignore;
   weights_out_value_tmp._iteration = weights_out_value_iteration_count;
   weights_out_value_tmp._data.mc_testbench_process_array_bounds("weights_out_value",testbench::weights_out_value_array_comp_first,testbench::weights_out_value_array_comp_last,15,0);
   // BEGIN: testbench output_mask control for field_name weights_out_value
   if ( testbench::weights_out_value_use_mask ) {
      weights_out_value_tmp._use_mask = true;
      weights_out_value_tmp._mask = testbench::weights_out_value_output_mask ;
   }
   // END: testbench output_mask control for field_name weights_out_value
   static int last_j_weights_out_value_out = 0;
   if (_capture_weights_out_value)
   {
      int cur_iter = weights_out_value_capture_count;
      ++weights_out_value_iteration_count;
      unsigned int chan_factor = 1;
      unsigned int ccs_scv_i,ccs_scv_j;
      ccs_scv_i = remaining_weights_out_value_golden;
      while (ccs_scv_i < weights_out.ac_channel<PackedInt<8UL, 16UL > >::debug_size()) {
         for (ccs_scv_j = last_j_weights_out_value_out; ccs_scv_j < chan_factor; ccs_scv_j++,ccs_scv_i++) {
            if (ccs_scv_i < weights_out.ac_channel<PackedInt<8UL, 16UL > >::debug_size()) {
               int weights_out_linear_idx = 0;
               for (int weights_out_idx_0 = 0; weights_out_idx_0 < 16; ++weights_out_idx_0)
                  weights_out_value_tmp._data[weights_out_linear_idx++] = weights_out.ac_channel<PackedInt<8UL, 16UL > >::chan[ccs_scv_i].PackedInt<8UL, 16UL >::value[weights_out_idx_0];
            } else {
               break;
            }
         }
         if (ccs_scv_j == chan_factor) {
            weights_out_value_golden.put(weights_out_value_tmp);
            last_j_weights_out_value_out = 0;
            weights_out_value_capture_count += chan_factor;
         } else {
            last_j_weights_out_value_out = ccs_scv_j;
         }
      }
      mc_testbench_util::process_wait_ctrl("weights_out_value",testbench::weights_out_value_wait_ctrl,ccs_wait_ctrl_weights_out_value.operator->(),cur_iter,weights_out_value_capture_count,1);
      testbench::weights_out_value_use_mask = false;
   }
   testbench::weights_out_value_ignore = false;
   testbench::weights_out_value_skip = false;
}
// ============================================
...
void mc_testbench::wait_on_input_required()
{
   ...
   while (atleast_one_active_input) {
      if (_capture_weights_in && ccs_weights_in->used() == 0) return;
      ...
      that->cpp_testbench_active.write(false);
      wait(ccs_weights_in->ok_to_put() | ccs_paramsIn_OY1->ok_to_put() | ccs_paramsIn_OX1->ok_to_put() | ccs_paramsIn_OY0->ok_to_put() | ccs_paramsIn_OX0->ok_to_put() | ccs_paramsIn_OC1->ok_to_put() | ccs_paramsIn_IC1->ok_to_put() | ccs_paramsIn_FX->ok_to_put() | ccs_paramsIn_FY->ok_to_put() | ccs_paramsIn_STRIDE->ok_to_put());
      that->cpp_testbench_active.write(true);
   }
}
// ============================================
...
void mc_testbench::capture_IN( ac_channel<ac_int<8, true > > &weights_in,  ac_channel<PackedInt<8UL, 16UL > > &weights_out,  ac_channel<Params > &paramsIn)
{
   that->capture_weights_in(weights_in);
   that->remaining_weights_out_value_golden =  weights_out.ac_channel<PackedInt<8UL, 16UL > >::debug_size();
...
void mc_testbench::capture_OUT( ac_channel<ac_int<8, true > > &weights_in,  ac_channel<PackedInt<8UL, 16UL > > &weights_out,  ac_channel<Params > &paramsIn)
{
   that->remaining_ccs_weights_in =  weights_in.ac_channel<ac_int<8, true > >::debug_size();
   if (testbench::enable_idle_sync_mode && that->remaining_ccs_weights_in != 0) {
      cout << "Warning: Testbench input channel 'weights_in' not empty. Remaining size: " << that->remaining_ccs_weights_in << endl;
   }
   that->capture_weights_out(weights_out);
...
void mc_testbench::exec_run( ac_channel<ac_int<8, true > > &weights_in,  ac_channel<PackedInt<8UL, 16UL > > &weights_out,  ac_channel<Params > &paramsIn)
{
   #ifndef CCS_SCVERIFY_USE_CCS_BLOCK
   that->cpp_testbench_active.write(true);
   capture_IN(weights_in, weights_out, paramsIn);
   WeightDoubleBuffer<4096, 16, 16>::run(weights_in, weights_out, paramsIn);
   ...
   capture_OUT(weights_in, weights_out, paramsIn);
   #else
   #endif
}
// ============================================
...
void mc_testbench::check_results()
{
   ...
   if (!_capture_weights_out_value) {
      cout<<"'weights_out_value' - warning, output was optimized away"<<endl;
   } else {
      _num_outputs_checked++;
      _failed |= weights_out_value_comp->check_results(weights_out_value_capture_count,testbench::weights_out_value_skip_noerr);
      _has_capture_counts |= !!(weights_out_value_capture_count);
   }
   cout<<endl;
...
void mc_testbench::wait_for_end() {
   ...
   if (weights_out_value_comp->get_compare_count() < weights_out_value_capture_count) {testbench_end_event.notify(1,SC_NS); return;}
...
void mc_testbench::run() {
   ...
   testbench::weights_in_ignore = false;
   testbench::weights_in_skip = false;
   testbench::weights_in_skip_quiet = false;
   testbench::weights_in_skip_once = false;
   testbench::weights_in_skip_noerr = false;
   testbench::weights_in_array_comp_first = -1;
   testbench::weights_in_array_comp_last = -1;
   testbench::weights_in_wait_ctrl.clear();
   weights_in_capture_count = 0;
   weights_in_iteration_count = 0;
   testbench::weights_out_value_ignore = false;
   testbench::weights_out_value_skip = false;
   testbench::weights_out_value_skip_quiet = false;
   testbench::weights_out_value_skip_once = false;
   testbench::weights_out_value_skip_noerr = false;
   testbench::weights_out_value_array_comp_first = -1;
   testbench::weights_out_value_array_comp_last = -1;
   testbench::weights_out_value_use_mask = false;
   testbench::weights_out_value_output_mask = ~0;
   testbench::weights_out_value_wait_ctrl.clear();
   weights_out_value_capture_count = 0;
   weights_out_value_iteration_count = 0;
   testbench::paramsIn_OY1_ignore = false;
   ...
   cout<<"   captured "<<weights_in_capture_count<<" values of weights_in"<<endl;
   cout<<"   captured "<<weights_out_value_capture_count<<" values of weights_out_value"<<endl;
